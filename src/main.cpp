...
void check_collision(World &world, Collision collision)
{
    ...
    auto q = world.query<With<Position &>>();

    for (auto &[pos] : q.all())
    {
        std::println("Collision Position: x: {} y: {}", pos.x, pos.y);
    }
}
                ...
                EventManager &event_manager, EntityManager &entity_manager)
{
    ...
    auto a = query.all();
    auto b = q2.all();
    for (auto &[entity, pos, vel, name] : a)
    {
...
int main()
{
    ...
    lands.build_plugins();
    Entity e = lands.add_entity(pos, vel, Name{.name = "Test"});
